...
def find_next_empty(puzzle):
    ...
    return None, None # if no spaces in puzle are empty(-1)
...
def is_valid(puzzle, guess, row, col):
    ...
    # and iterate overthe 3 values in the row/column
...
def solve_sudoku(puzzle):
    ...
    # step 1.1 : if there's nowhere left, then we're donw because we only allowed valid inputs
    if row is None:
        return True

    # step 2 : if there is a place to put a numberm then make a guess between 1 and 9
    for guess in range(1, 10):
        ...
        if is_valid(puzzle, guess, row, col):
            ...
            # step 4 : recursively call our funtion
